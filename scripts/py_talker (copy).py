...
def talker():
    ...
    for i in range(5):
        msg.data.append(q[i].item())
    msg.data.append(0.0)
    pub.publish(msg)
    time.sleep(5.0)
    msg.data.clear()
    for i in range(5):
        msg.data.append(np.pi/4)
    msg.data.append(0.0)
    pub.publish(msg)
    time.sleep(5.0)
    msg.data.clear()
    ...
    for k in range(5):
        time.sleep(5.0)
        msg.data.clear()
        rand = (np.random.random(6)-0.5*np.ones(6))*np.pi*np.ones(6)
        for i in range(5):
            msg.data.append(-rand[i])
        msg.data.append(0.0)
        pub.publish(msg)

    
    # rate1 = rospy.Rate(0.05)
    # rate1.sleep()
